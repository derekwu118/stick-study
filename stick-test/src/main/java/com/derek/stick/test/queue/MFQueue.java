package com.derek.stick.test.queue;
...
public class MFQueue extends AbstractQueue<byte[]> {
    ...
    public MFQueue(String queueName, String fileBackupDir) {
        ...
        this.writeBlock = new MFQueueBlock(index, MFQueueBlock.formatBlockFilePath(queueName, index.getWriteNum(),
                                                                                   fileBackupDir));
        if (index.getReadNum() == index.getWriteNum()) {
            this.readBlock = this.writeBlock.duplicate();
        } else {
            this.readBlock = new MFQueueBlock(index, MFQueueBlock.formatBlockFilePath(queueName, index.getReadNum(),
                                                                                      fileBackupDir));
        }
    }
    ...
    private void rotateNextWriteBlock() {
        ...
        writeBlock = new MFQueueBlock(index,
                                      MFQueueBlock.formatBlockFilePath(queueName, nextWriteBlockNum, fileBackupDir));
    ...
    private void rotateNextReadBlock() {
        ...
        if (nextReadBlockNum == index.getWriteNum()) {
            ...
            readBlock = new MFQueueBlock(index,
                                         MFQueueBlock.formatBlockFilePath(queueName, nextReadBlockNum, fileBackupDir));
        }
